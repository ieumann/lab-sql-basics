...
SELECT date, duration, COUNT(*) as num
FROM loan
...
### Query 20
# From the previous result, modify your query so that it returns only one row, with a column for incoming
# amount, outgoing amount and the difference.
SELECT SUM(CASE type WHEN 'PRIJEM' THEN amount ELSE 0 END) AS incoming_amount, # If type is equal to 'PRIJEM', then the value of amount is selected. If type is not equal to 'PRIJEM', then 0 is selected instead.
       SUM(CASE type WHEN 'VYDAJ' THEN amount ELSE 0 END) AS outgoing_amount, # END is used to end a conditional statement
       FLOOR(SUM(CASE type WHEN 'PRIJEM' THEN amount ELSE -amount END)) AS difference # Calculating the difference of the sums of incoming and outgoing
FROM trans
WHERE account_id = 396;
